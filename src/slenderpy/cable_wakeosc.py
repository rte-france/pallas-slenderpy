...
def solve(
    ...
    """
    M = wo.rho * cb.d**2 * wo.cl0 / (16.0 * np.pi**2 * wo.st**2 * cb.m)
    w = 2.0 * np.pi * wo.u * wo.st / cb.d
    ...
    for k in range(pm.nt):
        ...
        if fast:
            ...
            if M == 0.0:
                ...
                un[1:-1] = ht * vn[1:-1] + R1
                r[1:-1] = sp.linalg.solve_banded(
                    (1, 1), Yb, (R4 + A41 * un[1:-1] + (ht * bt) * vn[1:-1] - ht * R3)
                )
                q[1:-1] = ht * r[1:-1] + R3
            else:
                Xx = (
            ...
            X = np.concatenate(
                (un[1:-1], vn[1:-1], ub[1:-1], vb[1:-1], q[1:-1], r[1:-1])
            )
            ...
            un = np.concatenate(([0.0], Xn[0 * n : 1 * n], [0.0]))
            vn = np.concatenate(([0.0], Xn[1 * n : 2 * n], [0.0]))
            ub = np.concatenate(([0.0], Xn[2 * n : 3 * n], [0.0]))
            vb = np.concatenate(([0.0], Xn[3 * n : 4 * n], [0.0]))
            q = np.concatenate(([0.0], Xn[4 * n : 5 * n], [0.0]))
            r = np.concatenate(([0.0], Xn[5 * n : 6 * n], [0.0]))
