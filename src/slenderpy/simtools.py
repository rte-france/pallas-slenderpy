...
class Parameters:
    ...
    def _check_input(ns, t0, tf, dt, dr, los, pp):
        ...
        if dt < 0.0 or dt >= 0.5 * (tf - t0):
            raise ValueError(
                "input dt must be a positive float and much smaller than tf-t0"
            )
        if dr < 0.0 or dr < dt:
...
class Results:
    ...
    def to_json(self):
        ...
        out = {
            __stime__: self.data[__stime__].values.tolist(),
...
def multiplot(
    ...
    if len(res) == 1:
        ...
        cmap = cm.viridis(np.linspace(0.0, 1.0, len(res) + 2))[1:-1]
    ...
    for j in range(nc):
        ax[0, j].set_title(
            f"@ x={los[j] * Lref:.1E} m ({los[j] * 100.:.1f} %)", fontsize=fst
        )
        if not log:
