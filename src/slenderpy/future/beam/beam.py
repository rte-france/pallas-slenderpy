    ...
    def solve_dynamic(
        ...
        for k in range(parameters.nt):
            ...
            if approx_curvature:
                ...
                curvature_old = D2_border@y_old / np.sqrt((np.ones(nb_space) + ((FD.first_derivative(nb_space,ds))@y_old)**2) ** (3))
                ...
                while it < it_picard and error > tol_picard:
                    curvature_picard = D2_border@y_picard / np.sqrt((np.ones(nb_space) + ((FD.first_derivative(nb_space,ds))@y_picard)**2) ** (3))
...
class BeamEIVariable(Beam):
    ...
    def solve_dynamic(
        ...
        if approx_curvature:
            ...
            def curvature(y):
                return D2_border@y / np.sqrt((np.ones(nb_space) + ((D1@y)**2)) ** 3)
        M = self.mass * Id
        A = M + dt2**2 * K + BC
        B = M - dt2**2 * K
        ...
        for k in range(parameters.nt):
            ...
            while it < it_picard and error > tol_picard :
                ...
                bending_moment_picard = self._bending_moment_dynamic(curvature_picard, eta_picard)
                rhs = (
                    B @ v_old
                ...
                eta_new = ( eta_old + (diff - 0.5 * diff * np.abs(eta_picard))/ self.critical_curvature ) / ( 1 + 0.5*np.abs(diff)/ self.critical_curvature)
                eta_picard = eta_new 
                it += 1
